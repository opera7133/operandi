...
class OpenGraph implements Iterator
{
    ...
    private static function _parse($HTML)
    {
        ...
        foreach ($tags as $tag) {
            if (
                ...
                $key = strtr(
                    substr($tag->getAttribute("property"), 3),
                    "-",
                    "_"
                );
    ...
    {
        if (
            array_key_exists("latitude", $this->_values) &&
            array_key_exists("longitude", $this->_values)
        ) {
            return true;
        }
        ...
        foreach ($address_keys as $key) {
            $valid_address =
                $valid_address && array_key_exists($key, $this->_values);
        }
